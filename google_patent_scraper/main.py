...
class scraper_class:
    ...
    def process_patent_html(self,soup):
        ...
        try:
            ...
            description_all=description.find('div',itemprop="content")
            description_total=''
            for i,child in enumerate(description_all.div.children):
                if(child!='\n'):
                    num=int((i+1)/2)
                    description_text=child.get_text()
                    description_text=description_text.replace(': \n\n','@@@\n')
                    description_text=description_text.replace('; \n\n','@@@\n')
                    description_text=description_text.replace('\n\n','@@@\n')
                    description_text=description_text.replace('@@@','')
                    description_total+=description_text
            print(f'{description_total}')
            # Desc=[]
            # Desc_total=''
            # for i,child in enumerate(description_all.div.children):
            #     if(child!='\n'):
            #         num=int((i+1)/2)
            #         Claims_text=child.get_text()
            #         Claims_text=Claims_text.replace(': \n\n','@@@\n')
            #         Claims_text=Claims_text.replace('; \n\n','@@@\n')
            #         Claims_text=Claims_text.replace('\n\n','@@@\n')
            #         Claims_text=Claims_text.replace('\n','')+'\n'
            #         text_dict={f'Claim_{num}':{'Claim_text':Claims_text}}
            #         Claims.append(text_dict)
            #         #print(child.get_text(),f'{num}')
            #         c=0
            #         for j in child.find_all('a',href=re.compile("http")):
            #            c+=1
            #            Claim_images=j.attrs['href']
            #         #    if (Claims_text.find('; ')!=-1 and Claims_text.find(': ')>Claims_text.find('; ') and Claims_text.find(': ')!=-1) or (Claims_text.find('; ')!=-1 and Claims_text.find(': ')!=-1):
            #         #        print(Claims_text.find('; '))
            #         #        Claims_text=Claims_text.replace('; ',f':##Claim_images_{c}##')
            #         #    else:
            #         #        Claims_text=Claims_text.replace(': ',f':##Claim_images_{c}##')
            #            Claims_text=Claims_text.replace('@@@',f':##Claim_images_{c}##',1)
            #            Claims[num-1][f'Claim_{num}']['Claim_text']=Claims_text
            #            Claims[num-1][f'Claim_{num}'][f'Claim_images_{c}']=Claim_images
            #            Claim_images=''
            #           # Claims = [{f'Claim_{num}':{{'Claim_text':Claims_text},{f'Claim_images{j}':Claim_images}}}]
            #         claims_total+=Claims_text
            # print(claims_total)
        except:
            ds=['have bug please check ds']
            print(ds)
        # Worldwide publication
        #try:
        #    worldwide_current = [{'worldwide_current': x['data-result']} for x in soup.find_all('span', class_='application-timeline')]
        #except:
        #    worldwide_current = []
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        #  Get abstract 
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        try:
            Claim=soup.find('section',itemprop="claims")
            Claims_all=Claim.find('div',itemprop="content")
            Claims=[]
            claims_total=''
            for i,child in enumerate(Claims_all.div.children):
                if(child!='\n'):
                    num=int((i+1)/2)
                    Claims_text=child.get_text()
                    Claims_text=Claims_text.replace(': \n\n','@@@\n')
        ...
        try:
            ...
            worldwide_current_list = soup.find_all('li', itemprop='application')
            for worldwide in worldwide_current_list:
                current_app = worldwide.find('span',itemprop='applicationNumber').get_text()
                worldwide_current.append(current_app)
        except:
            worldwide_current =[]
        ...
        if self.return_abstract:
            ...
            # Get text 
            if abstract:
                abstract_text=abstract.find('div',class_="abstract").get_text()
        ...
        #         'worldwide_current': worldwide_current,
        #         'pub_date':pub_date,
        #         'priority_date':priority_date,
        ...
        return({'title': title_text,
                ...
                # 'assignee_name_orig':assignee_name_orig,
                # 'assignee_name_current':assignee_name_current,
                # 'worldwide_current': worldwide_current,
                # 'pub_date':pub_date,
                # 'priority_date':priority_date,
                # 'grant_date':grant_date,
                # 'filing_date':filing_date,
                # 'forward_cite_no_family':forward_cites_no_family,
                # 'forward_cite_yes_family':forward_cites_yes_family,
                # 'backward_cite_no_family':backward_cites_no_family,
                # 'backward_cite_yes_family':backward_cites_yes_family,
                'abstract_text':abstract_text,
               'Claims':Claims,
